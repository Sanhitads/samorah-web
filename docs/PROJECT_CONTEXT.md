...
- **Next:** **Phase 8 — building the platform** (Decision 23; `PAGE_ARCHITECTURE.md`
  + `ARCHITECTURAL_PRINCIPLES.md`). Build order: ✅ step 1 architecture docs ·
  ✅ step 2 data models (`src/platform/`, framework-agnostic) · ✅ step 3
  **Theme / Design-System Registry** (`theme.ts` pure registry with `extends`
  inheritance + per-theme accent + category/version; CSS generated by
  `themeStylesheet.ts`, mounted once in the root layout) → **now: step 4 Asset
  System** (`AssetImage` + resolver), then relationship engine · section engine ·
  templates · chapter pages → air → shop. Homepage untouched; its CTAs
  (chapters · /shop · /bundles · /collections/air) 404 until pages exist.
