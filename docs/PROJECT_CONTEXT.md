...
- **Next:** **Phase 8 — building the platform** (Decision 23; `PAGE_ARCHITECTURE.md`
  + `ARCHITECTURAL_PRINCIPLES.md`). Build order — **all engines complete**:
  ✅ step 1 architecture docs · ✅ step 2 data models (`src/platform/`,
  framework-agnostic) · ✅ step 3 **Theme / Design-System Registry** · ✅ step 4
  **Asset System** (`AssetImage` + resolver, render-plan abstraction) · ✅ step 5
  **Relationship Engine** (time-aware, bidirectional graph + type registry) ·
  ✅ step 6 **Section Engine** (registry + renderer + shell + error boundary +
  CMS-grade manifest) · ✅ step 7 **Template Engine** (versioning · `extends`
  inheritance · manifest · named slots · composition rules + validation;
  `coreTemplates` = Editorial Chapter + Limited Edition) · ✅ step 8 **Page
  Engine** (`pageResolver` documented pipeline + `PageView`/`buildPageMetadata`;
  cache-policy · page-events · middleware seams · preview-session · page-manifest
  contracts — Decision 24). **Architecture frozen here.** → **now: step 9 —
  Chapter Pages** (`/chapters/[slug]`, real data through `PageView`), then Air ·
  Campaign · Journal · Shop · CMS. Homepage untouched; its CTAs (chapters · /shop ·
  /bundles · /collections/air) 404 until pages exist.
...
23. **Phase 8 reframed as a PLATFORM, not pages — section-composition architecture (Shopify-grade).** Full spec in **`PAGE_ARCHITECTURE.md`** (new doc SoT). Hierarchy **Platform → Experience → Page → Section → Block → Atom**: pages belong to an **Experience** (candle-chapters · air-chapters · shop · journal · campaign · gift; product-agnostic `ExperienceType`); a **Page** = rich metadata (slug · template · SEO engine · OG · canonical · breadcrumb · resolved navigation graph · status `draft/preview/published/archived` · publish schedule · palette · campaign · locale) + an ordered `Section[]`; a **Section** = `type` **+ `variant`** (no per-look duplication) + a CMS envelope (**theme token** · **editorial mood** · spacing · animation · layout · background · campaign overrides · trackingId · schedule · visibility) + `settings` + child **Blocks**; **Block Registry** mirrors the Section Registry; **Atoms** (ProductCard · AssetImage · PriceTag …). **Theme Tokens** = named palettes (warm-ivory · dark-library · forest · clay · sand · monsoon · winter · campaign-*) — replaces light/dark, re-themes per chapter/campaign by reference. **Asset model** (Media Library) replaces raw URLs — responsive desktop/tablet/mobile · video/poster · **image role** (hero/support/detail/portrait/lifestyle/architecture/texture/people/macro/transition/background) · focal point · dominant colour · photographer/location/season/campaign/licence/alt; gradient placeholders become placeholder Assets → Cloudinary by data swap. **Product Relationship engine** — products belong to MANY experiences (chapter · campaign · bundle · gift-guide · editorial-world · related · mood · season · homepage), not one collection. **Editorial World** promoted to a reusable content type (the §7 model) powering all galleries. The homepage components are **registered** into the section library (not rewritten). **Templates:** A Editorial Chapter (`/chapters/[slug]`, per-Volume theme, no filters/grids), B Air Chapters / The Hours (`/collections/[slug]` — cinematic hero → Shared Hours/Room alternating Hour Blocks → Divider → Private Hours/Linen → Future Volume teaser; each Hour a diary entry → its Room/Linen spray), C Shop (functional PLP: filters · grid · pagination). **Build order (no UI before data models):** architecture doc → data models → section engine → asset system → product relationships → template engine → CMS config → chapter pages → air → shop → admin. **[P8] = load-bearing core now** (hierarchy types · page metadata · section engine+variants+theme tokens · block registry · asset model+AssetImage · product-relationship model · Editorial World content type · ProductCard · Template A end-to-end). **[Future] = designed-for, deferred** (analytics wiring · search index · localization · admin GUI/visual editor · CMS preview · scheduled activation · media-library DB). · Composition over hard-coding; scales beyond the first site without retrofits. · 2026-06-26 · 8 · All page/CMS architecture; Phases 8–10. *(Architecture doc written; awaiting approval to build data models.)* **v2 additions (2026-06-26):** hierarchy gains an optional **Journey** level (Experience→Journey→Page); **DesignSystem version** object (version/token/component/typography — future redesigns don't break old pages); content workflow expanded to **Draft→Review→Approved→Scheduled→Published→Archived**; **global Taxonomy** model (mood·season·collection·experience·occasion·material·color·ingredient·space·emotion — everything references it); **Commerce domain** documented as independent from editorial (pricing·inventory·bundles·offers·shipping·taxes·availability·region·currency; editorial reads a *projection*, never raw money/stock); **Personalization** model (preferred mood/chapter · viewed/visited — model now, activation later); lightweight **Event** model + `emitEvent` seam (viewed·clicked·subscribed·purchased… — sink later). **Build order tweak:** Theme System *before* Section Engine. Git-style content version history explicitly **NOT** now (variant + campaign override; belongs in Admin CMS). New constitution doc **`ARCHITECTURAL_PRINCIPLES.md`** added.
...
| `ARCHITECTURAL_PRINCIPLES.md` | ✅ v2 (2026-06-26) | **The constitution** — 27 non-negotiable principles in 5 groups (Structure & domains · Content & data · Composition & craft · Experience for everyone · Longevity & operations). Incl. Experience before Product · Editorial before Commerce · **Domains remain independent** · **API before Interface** · Assets before URLs · Relationships over Duplication · Theme Tokens over Colours · Taxonomy over Free Text · **AI Ready by Design** · One Source of Truth · every Component CMS-ready · Animation serves Storytelling · **Performance before Decoration** · **Progressive Enhancement** · Accessibility · **Security by Default** · Versions over Breakage · **Content Outlives Campaigns** · Events over Guesswork · **Document Decisions** · **Commerce thoughtfully integrated** (softened from "never a shop") · ends with **Build for the Next Five Years, not the Next Five Days**. Read before touching code; prevents drift |
| `PAGE_ARCHITECTURE.md` | ✅ v2 (2026-06-26) | **Platform architecture SoT** — Platform→Experience→**Journey**→Page→Section→Block→Atom; **DesignSystem versioning**; section variants + theme tokens + editorial mood; **global Taxonomy**; Asset/Media-Library model; **Commerce domain boundary**; Product-Relationship engine; Editorial World content type; content workflow (Draft→Review→Approved→Scheduled→Published→Archived); **Personalization** + **Event** models; SEO/navigation; the 3 templates; build order. Governs Phases 8–10 (Decision 23) |
...
### Platform engines — Template + Page (Decision 24 · 2026-06-29)

Phase 8 build steps 7–8 complete the engine layer; **architecture is now frozen** — from here we prove it by building real pages, not extend it.

- **Template Engine** (`platform/template.ts`, `coreTemplates.ts`): a template is the default ordered `Section[]` for an experience. **Versioned** (registry keyed `${id}@${version}` + LATEST; pages pin a revision) · **inheritance** (`extends` → `resolveTemplate` walks the chain, merging sections by id + capabilities/intent/rules, cycle-guarded) · **manifest** (label/description/previewAsset/category/capabilities) · **named slots** (`opening|narrative|product|atmosphere|closing` on each section — structure over position) · **composition rules** + `validateComposition()` (requiredTypes/requiredSlots/maxOccurrences → `ValidationIssue[]`) · **intent** (audience/seoIntent/editorialGoal/commerceEmphasis). Core templates: `EDITORIAL_CHAPTER_TEMPLATE` (v1) + `LIMITED_EDITION_CHAPTER` (extends it, overrides hero + gallery only).
- **Page Engine** (`platform/pageResolver.ts` + `components/page/`): `resolvePage` → `{ sections, context, cachePolicy }` via a **documented pipeline** — *Lifecycle → [before-resolve seam] → Template → Sections → Relationships → Navigation → SEO → Context → Cache → [after-resolve seam] → Render*. UI layer is thin: `PageView` (resolve → JSON-LD → SectionRenderer) + `buildPageMetadata` (`seo` → Next `Metadata`, OG via asset resolver). Six forward contracts shipped as **shape, not implementation**: **cache policy** (`static|isr|dynamic|preview`, on the page model so strategy survives a framework change; `resolveCachePolicy` derives a default from lifecycle) · **page events** (`viewed|scrolled|completed|cta`, analytics seam) · **middleware seams** (`beforeResolve`/`afterResolve` — localization/auth/experiment/campaign in, analytics/logging/personalization out; loops exist, no hooks ship) · **preview session** (`PreviewSession{draftId,editor,expiresAt}` — the shape `preview: boolean` grows into) · **page manifest** (displayName/previewAsset/purpose/owner for the CMS).
- **Cross-domain cleanup:** `ValidationIssue` defined once in `platform/primitives`, shared by section + template validation. Platform layer remains framework-agnostic (verified). Homepage byte-identical (5.26 kB) — engines tree-shake out until a route uses `PageView` (step 9).
...
### Checkout — GST-compliant money engine + payment/order pipeline (Decision 26 · 2026-07-04)

Beat 1 is **hardened into a production-grade commerce architecture** before Beat 2 (no rewrite; the money layer is the base). Analysis + build:

**Central config (highest priority) — `config/commerce.ts`** is the ONE source of truth: `COMMERCE` (brand · legalName · **GSTIN** · registered address/state · currency · support · policy/return-window/cancellation + policy hrefs) · **`TAX_CLASSES`** (per-type **HSN + GST rate** — candle 3406/12%, room/linen spray 3307/18%, wax_melt/accessory/gift_set) · `SHIPPING` (free ≥ ₹1,499 else ₹99 fallback) · **`INVOICE`** (prefix + `financialYear()` + `format()` → `SAM/26-27/000001`) · **`RAZORPAY`** env placeholders. Every module reads from here.

**Money engine — `lib/commerce.ts` `computeOrderTotals(lines,{state,giftCard,couponCode})`** is the GST-compliant pipeline, in order: **line totals → promotions (allocated PER LINE) → per-line taxable + GST extraction at the line's OWN HSN rate → aggregate → shipping + shipping GST (composite supply, principal rate) → CGST/SGST vs IGST split → gift card (payment) → amount payable.** GST is **never** extracted from a discounted cart total — verified: candle 12% + 2 sprays 18% → **₹248.56** (per-line), not ₹203.25 (single-rate); ₹99 shipping @ 12% → GST ₹10.61. `buildCartSummary` (cart) and `calculateOrderTotals` (checkout) are **thin views** over this engine — **unchanged public API**. Returns a per-line breakdown (HSN · rate · taxable · GST) = the future **immutable invoice snapshot**.

**Promotion engine — `lib/promotions.ts`** — the Discovery Composition is now one RULE (`DISCOVERY_COMPOSITION`, 15% per complete set, allocated per line); `COUPONS` registry + `computePromotions()` support percentage/fixed/free-shipping for Welcome/Festival/Influencer/campaign codes (none active — architecture only). Coupons are discounts; **gift cards are payment** (separate slot, after tax).

**Order architecture — `lib/orders.ts` (types only; the webhook is the only writer)** — `OrderStatus` lifecycle (pending_payment → paid → packed → shipped → delivered → cancelled → refund_requested → refunded) · `OrderRecord` with **billing + shipping addresses**, **business GST** (companyName + GSTIN), **gift fields** (`isGift · giftMessage · giftRecipientName · hideInvoice`), order notes, guest flag · **immutable** `OrderLineSnapshot` (name · variant · edition · chapter · HSN · GST · discount · image — so a later rename never changes a past invoice) + `OrderTotalsSnapshot` · `ORDER_EMAILS` (confirmation/payment/shipment/delivered/refund) · `CartValidation` (server re-price contract).

**Checkout UI (`/checkout`, editorial preserved):** guest-first · shipping address · **"billing same as shipping"** toggle + separate billing form · optional **GST business invoice** (company + GSTIN validated `^\d{2}[A-Z]{5}\d{4}[A-Z][A-Z\d]Z[A-Z\d]$`) · **order notes** · **legal-consent checkbox** (pay button disabled until accepted) · **delivery-estimate placeholder** ("Available after payment"). Zod validation; cart never cleared before a real payment.

**v2 production hardening (2026-07-04):**
- **Integer paise throughout** — `lib/money.ts` (`toPaise/formatPaise/extractPaise`); the engine computes in integer paise (no float drift), returns paise; **UI is the only rupee conversion**. Snapshots store paise.
- **Promotion stacking** — every promotion declares `priority · stackable · exclusive · combinableWith[]`; **deterministic** application (sort by priority → greedy compatibility); free-shipping promos always stack; composition + WELCOME10 correctly *don't*. Each application yields an **immutable `PromotionSnapshot`** (name · campaign · version · rule · amount).
- **`taxVersion`** (`GST_2026_01`) stamped on every order → historical invoices never depend on live tax config.
- **Payment states** Pending / Failed / **Expired** distinct; **Razorpay retry reuses** the open order (`canReuseRazorpayOrder`) — never a duplicate.
- **Audit timeline** (`OrderEventRecord`: created→validated→payment_started→webhook_received→invoice_generated→shipment_created→delivered→refunded).
- **Reservation expiry** lifecycle (`StockReservation` + `isReservationExpired` + 15-min TTL + documented release cron).
- **Partial shipment** (`Shipment[]` per order), **cancellation window** (`canCancel`, cancellable ≤ `packed`), **refund** (full/partial `Refund`).
- **Composite-supply shipping** — shipping GST now attracts the **highest** rate present in the cart (all-candle → 12%, candle+spray → 18%), not the dominant-by-value rate. A fully-discounted / empty order carries no shipping and no GST.
- **Zero-rupee bypass** — `requiresPayment(payable)`; when a gift card/loyalty covers the total (₹0), the server confirms the order internally (invoice · stock · emails) and **never opens Razorpay** (it can't process ₹0).
- **PIN ↔ state cross-check** — `pinStateMismatch()` flags a confident PIN-vs-selected-state mismatch (Delhi PIN + "Maharashtra" → error) so CGST/SGST vs IGST is never mis-computed; unknown prefixes never block. Wired into checkout validation.
- **Commerce Test Matrix** — living **`docs/COMMERCE_TEST_MATRIX.md`** (the definition-of-done; every new commerce feature adds cases). `vitest` (`npm test`), **22 passing**: single-product/mixed/inter-state GST (guards the single-rate ₹203.25 regression) · composite-supply shipping (highest rate) · line-item taxes = header (no 1-paisa drift) · 100%-discount free order · gift-card zero-payment bypass · composition · promotion stacking/priority · reservation expiry · retry reuse · cancellation window · FY invoice · PIN↔state · GSTIN.
- **Provenance stamps** — every `OrderTotals`/order freezes `taxVersion` (`GST_2026_01`) · `pricingVersion` (`PRICE_2026_07`) · `commerceVersion` (`1.0`), so you know exactly which rules produced a two-year-old invoice. **Audit events** carry `id (uuid) · event · timestamp · actor · payload`.
- **Self-documenting commerce docs** (new): `COMMERCE_ARCHITECTURE.md` (engine · pipeline · versions · money precision) · `GST_COMPLIANCE.md` (per-line rounding rule — one rule, frozen · composite supply · CGST/SGST/IGST) · `INVOICE_RULES.md` (FY numbering · immutable snapshot) · `ORDER_LIFECYCLE.md` (status · payment · events · reservation · cancel · refund · shipment) · `PAYMENT_FLOW.md` (locked Razorpay/webhook flow) · `COMMERCE_TEST_MATRIX.md`.

**Deferred — Checkout Beat 2 (needs Razorpay keys + `npm i razorpay` + orders-table wiring). Locked flow:** Cart → **server re-validates totals + promotions** (composition still exactly 3 · same vessel · eligible · discount valid, else drop it and error — never trust the client) → `create-order` (server, `RAZORPAY_*` env) → Razorpay Checkout → **webhook = order source of truth** (BRD USD P5/P6; ALL side effects here, `/verify` only redirects; **idempotent** `UPDATE … WHERE idempotency_key IS NULL` + `webhook_logs`) → persist `orders`/`order_items` **immutable snapshots** → **allocate FY invoice number** (per-FY Postgres sequence, only on payment success) → **stock_reservations** (15-min hold) / deduct → **Shiprocket** shipment (serviceability/rate by **PIN → validated state overrides the manually-selected state for GST**, replacing the flat estimate — no hardcoded PIN table) → confirmation **email** (React Email + Resend, Phase 14) → Thank-You page. **No business logic runs on the client after payment success.** Failed payment never clears the cart (retry / edit address / continue). Variants gain their own HSN/GST columns (override `TAX_CLASSES`). **Confirm before first live order:** GSTIN · registered state · HSN/rates w/ CA · free-ship threshold.
