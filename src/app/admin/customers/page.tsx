    ...
    <main className="admin">
      ...
      <div className="admin__table-wrap">
        <table className="admin__table admin__table--board">
          <thead><tr><th>Customer</th><th>Segment</th><th>Health</th><th>Orders</th><th>Value</th><th>Since</th><th>Tags</th><th></th></tr></thead>
          <tbody>
            {customers.map((c) => (
              <tr key={c.id}>
                <td><Link href={`/admin/customers/${c.id}`} className="od-link">{c.name}</Link><div className="admin__muted">{c.email}</div></td>
                ...
                <td className="admin__mono">{money(c.ltv)}{c.aov ? <div className="admin__muted">avg {money(c.aov)}</div> : null}</td>
                <td className="admin__muted">{monthYr(c.createdAt)}</td>
                <td><div className="adm-badges">{c.newsletter ? <span className="adm-badge" data-b="cod">news</span> : null}{c.wholesale ? <span className="adm-badge" data-b="gst">wholesale</span> : null}{c.tags.slice(0, 3).map((t) => <span key={t} className="adm-badge">{t}</span>)}</div></td>
                <td><Link href={`/admin/customers/${c.id}`} className="ff-btn">View</Link></td>
              </tr>
            ))}
            {customers.length === 0 ? <tr><td colSpan={8} className="admin__empty">No customers match{anyFilter ? " these filters" : ""}.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </main>
