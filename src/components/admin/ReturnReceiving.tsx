...
import type { ReturnReceivingModel } from "@/services/returnReceivingService";

/**
 * Receiving & Inspection panel (Phase 1B-1). Collects PHYSICAL FACTS only — received / restockable /
 * damaged / note per variant — never raw stock ±. Live reconciliation (restockable + damaged = received,
 * received ≤ outstanding) disables Commit locally; the DB CHECKs + commit_receipt stay authoritative.
 * Restock flows only through the receipt path; "Close receiving" is the authoritative boundary.
 */
type Draft = { received: string; restockable: string; damaged: string };

export function ReturnReceiving({ model, canOperate }: { model: ReturnReceivingModel; canOperate: boolean }) {
  ...
  // Local validation mirrors the DB invariants so the operator gets instant feedback.
  ...
  const submitReceipt = async () => {
    ...
    try {
      const res = await fetch("/api/admin/returns/receive", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ returnId: model.returnId, note: note || undefined, lines }),
      });
  ...
  const closeReceiving = async () => {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/admin/returns/receiving/close", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ returnId: model.returnId }),
      });
      const body = await res.json().catch(() => ({}));
  ...
  return (
    <div className="rcv">
      {/* Status banners */}
      {model.closed ? (
        <p className="rcv-banner rcv-banner--done">🔒 Receiving closed{model.closedByName ? ` by ${model.closedByName}` : ""}. Final shortage is fixed per variant below.</p>
      ) : model.legacyRestocked ? (
        <p className="rcv-banner rcv-banner--warn">This return was already restocked by the legacy path — receipt-driven receiving is disabled to prevent double restock.</p>
      ) : model.inconsistent ? (
        <p className="rcv-banner rcv-banner--warn">Inconsistent state: this return&apos;s resolution does not require a physical return, yet it is in a receiving state. Set resolution to “Return Required” or resolve without receiving.</p>
      ) : !model.requiresPhysicalReturn ? (
        <p className="rcv-banner rcv-banner--muted">This resolution does not require goods back — no receipt is needed and no stock will change.</p>
      ) : !model.canReceive ? (
        <p className="rcv-banner rcv-banner--muted">Receiving is available once the return is in a physical receiving state (Received / Inspection).</p>
      ) : null}

      {/* Per-variant reconciliation table */}
      <div className="admin__table-wrap">
        <table className="admin__table rcv-table">
          <thead>
            <tr>
              <th>SKU</th><th>Expected</th><th>Received</th><th>Outstanding</th>
              <th>Restockable</th><th>Damaged</th>{model.closed ? <th>Final missing</th> : null}
            </tr>
          </thead>
          <tbody>
            {model.variants.map((v) => (
              <tr key={v.variantRef}>
                ...
                <td className="admin__mono">{v.outstanding}</td>
      ...
      {/* Receipt-entry form — physical facts only, no stock ± */}
      {model.canReceive && canOperate ? (
        <div className="rcv-entry">
          <h3 className="od-card__title">Record a physical receipt</h3>
          <div className="admin__table-wrap">
          ...
          <div className="rcv-actions">
            ...
            <button className="admin__btn admin__btn--ghost" onClick={closeReceiving} disabled={!model.canClose || busy}
              title={model.openDraftCount > 0 ? "Commit or void open drafts first" : model.canClose ? "" : "Only from a receiving state"}>
              Close receiving
            </button>
          </div>
      ...
      {/* Receipt history + movement linkage */}
